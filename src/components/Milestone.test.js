...
it('should render the list of milestones as a table', () => {
  expect(wrapper.find(TableBody).find(TableRow).length).toEqual(mockMilestones.length);
});

it('should render the id in the first cell', () => {
  const firstRow = wrapper.find(TableBody).find(TableRow).first()
    .find(TableCell).map(x => x.children().text());
...
it('should render the description in the third cell', () => {
  const firstRow = wrapper.find(TableBody).find(TableRow).at(2)
    .find(TableCell).map(x => x.children().text());
